...
if ($groupid == '911'){
    ...
    $modelmenu = \app\models\MsMenu::find()
                ...
                ->where(['ua.GroupID' => $groupid,'mn.IsAktif' => 1,'Tipe' => 'RPT'])
                ->orderBy(['mn.MenuID'=>SORT_ASC])->all();
}
?>
<aside class="control-sidebar control-sidebar-dark" style="margin-top:-50px;">
    ...
    foreach ($modelmenu as $menuuser)
    {
        ...
        if ($parents == $menuuser->MenuID){
            ...
            $stringMenu = $stringMenu . '<ul class="sidebar-menu">'.
                    '<li class="treeview"><a href="#"><i class="'.$icon.'"></i> <span>'.$menuuser->MenuName.'</span></a>';
        }else{
            ...
            $stringMenu = $stringMenu .
                    '<ul class="treeview-menu">'.
                        '<li><a class="solTitle" href=index.php?r='.$menuuser->LinkAddress.'><span class="'.$iconA.'"></span>'.$menuuser->MenuName.'</a></li>'.
                    '</ul>';
        }
    }
    echo $stringMenu;
...
//    echo \yii\helpers\Html::a('Your Link name','index.php?r=master/master-area', [
//                'title' => Yii::t('yii', 'Close'),
//                    'onclick'=>"$('#ContentBody').dialog('open');//for jui dialog in my page
//                     $.ajax({
//                        type     :'POST',
//                        cache    : true,
//                        url  : 'index.php?r=master/master-area',
