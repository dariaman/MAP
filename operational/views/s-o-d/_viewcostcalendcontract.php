...
$queryGetDPPTHP = "DECLARE @gp DECIMAL(18,2),
		...
		SET @gp = (SELECT Amount AS Gapok FROM dbo.SOCostCalc WHERE SODID = '$SODID' AND BiayaID = 'GP000001')		

		SET @bpjs =	(SELECT ISNULL(sum(((Amount*@gp)/100)),0) AS Amount
					FROM dbo.SOCostCalc
					LEFT JOIN dbo.MasterBiaya ON MasterBiaya.BiayaID = SOCostCalc.BiayaID
					WHERE SODID = '$SODID' 
					AND SeqNo <= 1600 
					AND SUBSTRING(dbo.SOCostCalc.BiayaID,0,5) = 'BPJS' )

		SET @thp = (SELECT Jumlah = (Sum(cc.Amount)-(select Amount from SOCostCalc where  SODID='$SODID' and BiayaID='UMP00001'))
					from SOCostCalc cc
